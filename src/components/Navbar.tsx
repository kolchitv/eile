    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-amber-200/80 shadow-sm shadow-amber-500/5">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between h-16 gap-2 sm:gap-3">
          ...
          <div className="flex items-center bg-amber-100/80 p-1 rounded-2xl border border-amber-200/80 shrink-0 gap-0.5 sm:gap-1 shadow-2xs">
            {levels.map((lvl) => {
              ...
              return (
                <button
                  ...
                  className={`px-2 sm:px-3 py-1 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 shrink-0 cursor-pointer ${
                    isActive
                      ? 'bg-gradient-to-r from-emerald-700 to-teal-700 text-white shadow-sm shadow-emerald-800/25 border border-emerald-600 ring-1 ring-emerald-400/40'
                      : 'text-slate-700 hover:text-slate-950 hover:bg-white/80'
                  }`}
                  title={`${lvl} - ${language === 'ar' ? levelLabels[lvl].nameAr : language === 'fr' ? levelLabels[lvl].nameFr : levelLabels[lvl].nameEn}`}
                >
                  <span
                    className={`w-2 h-2 rounded-full shrink-0 ${
                      isActive ? 'bg-amber-300' : 'bg-amber-400/80'
                    }`}
                  />
                  <span>{lvl}</span>
                  {isActive && (
                    <span className="text-[10px] opacity-90 hidden xl:inline font-medium">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
            ...
            <div className="relative flex items-center bg-amber-100/80 rounded-xl p-0.5 border border-amber-300/80 text-xs font-bold shrink-0">
              <button
                ...
                className={`px-1.5 sm:px-2 py-1 rounded-lg transition-all cursor-pointer ${language === 'fr' ? 'bg-white text-slate-900 shadow-2xs font-extrabold' : 'text-slate-600 hover:text-slate-900'}`}
              >
                FR 🇫🇷
              </button>
              <button
                ...
                className={`px-1.5 sm:px-2 py-1 rounded-lg transition-all cursor-pointer ${language === 'ar' ? 'bg-white text-slate-900 shadow-2xs font-extrabold' : 'text-slate-600 hover:text-slate-900'}`}
              >
                عربي 🇸🇦
              </button>
              <button
                ...
                className={`px-1.5 sm:px-2 py-1 rounded-lg transition-all cursor-pointer ${language === 'en' ? 'bg-white text-slate-900 shadow-2xs font-extrabold' : 'text-slate-600 hover:text-slate-900'}`}
              >
                EN 🇬🇧
              </button>
            </div>
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-1.5 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:from-amber-600 hover:to-orange-600 text-white px-3 py-1.5 rounded-xl text-xs font-black shadow-sm shadow-amber-500/25 active:scale-95 transition-all shrink-0 cursor-pointer"
            >
